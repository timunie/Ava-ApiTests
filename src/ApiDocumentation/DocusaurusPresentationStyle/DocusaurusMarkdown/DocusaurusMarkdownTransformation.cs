...
namespace DocusaurusPresentationStyle.DocusaurusMarkdown
{
    ...
    /// presentation style.
    /// </summary>
    public class DocusaurusMarkdownTransformation : TopicTransformationCore
    {
        ...
        protected override XDocument RenderTopic()
        {
            if (pageTemplate == null)
                pageTemplate = LoadTemplateFile(TopicTemplatePath, null);
            ...
            if (!IsMamlTopic)
            {
                ...
                document.Root.Add(new XElement("file",
                    new XAttribute("name", ReferenceNode.Element("file")?.Attribute("name")?.Value)));
            }
        ...
        private static void RenderApiEnumerationMembersList(TopicTransformationCore transformation)
        {
            ...
            var thisTransform = (DocusaurusMarkdownTransformation)transformation;
