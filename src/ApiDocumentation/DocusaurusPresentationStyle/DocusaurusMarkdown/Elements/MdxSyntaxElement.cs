...
namespace DocusaurusPresentationStyle.DocusaurusMarkdown.Elements
{
    ...
    /// </summary>
    public class MdxSyntaxElement : Element
    {
        ...
        public MdxSyntaxElement(string baseSourceCodeUrlArgName) : base("syntax")
        {
        ...
        private static void RenderSyntaxSections(TopicTransformationCore transformation, XElement element, XElement content)
        {
            ...
            var tabs = new XElement("Tabs", new XAttribute("groupId", "api-code-preview"), "\n");
            
            foreach(var div in codeDivs)
            {
                string codeLanguage = div.Attribute("codeLanguage")?.Value ?? "Unknown";

                string displayName = codeLanguage;
                ...
                var tab = new XElement("TabItem", new XAttribute("value", codeLanguage), new XAttribute("label", displayName), "\n");

                tab.Add($"```{codeLanguage}", "\n");

                if(div.Attribute("phantom") != null)
                    tab.Add(new XElement("include", new XAttribute("item", "noCodeExample")));
                else
                {
                    if(div.Attribute("codeLanguage").Value == "XAML")
                        RenderXamlSyntaxBlock(transformation, div, tab);
                    else
                        transformation.RenderChildElements(tab, div.Nodes());
                }
