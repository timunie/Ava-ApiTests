---
title:DefaultValueConverter Constructor
description: Autogenertated API Reference page
hide_table_of_contents: false
---
# DefaultValueConverter Constructor


Initializes a new instance of the <a href="T_Avalonia_Data_Converters_DefaultValueConverter">DefaultValueConverter</a> class



## Definition
**Namespace:** <a href="N_Avalonia_Data_Converters">Avalonia.Data.Converters</a>  
**Assembly:** Avalonia.Base (in Avalonia.Base.dll) Version: 11.2.4

**C#**
``` C#
public DefaultValueConverter()
```
**VB**
``` VB
Public Sub New
```
**F#**
``` F#
new : unit -> DefaultValueConverter
```



## See Also


#### Reference
<a href="T_Avalonia_Data_Converters_DefaultValueConverter">DefaultValueConverter Class</a>  
<a href="N_Avalonia_Data_Converters">Avalonia.Data.Converters Namespace</a>  
