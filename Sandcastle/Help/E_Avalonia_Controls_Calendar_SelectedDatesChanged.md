---
title:SelectedDatesChanged Event
description: Autogenertated API Reference page
hide_table_of_contents: false
---
# SelectedDatesChanged Event




## Definition
**Namespace:** <a href="N_Avalonia_Controls">Avalonia.Controls</a>  
**Assembly:** Avalonia.Controls (in Avalonia.Controls.dll) Version: 11.2.4

**C#**
``` C#
public event EventHandler<SelectionChangedEventArgs> SelectedDatesChanged
```
**VB**
``` VB
Public Event SelectedDatesChanged As EventHandler(Of SelectionChangedEventArgs)
```
**F#**
``` F#
member SelectedDatesChanged : IEvent<EventHandler<SelectionChangedEventArgs>,
    SelectionChangedEventArgs>
```



#### Value
<a href="https://learn.microsoft.com/dotnet/api/system.eventhandler-1" target="_blank" rel="noopener noreferrer">EventHandler</a>(<a href="T_Avalonia_Controls_SelectionChangedEventArgs">SelectionChangedEventArgs</a>)

## See Also


#### Reference
<a href="T_Avalonia_Controls_Calendar">Calendar Class</a>  
<a href="N_Avalonia_Controls">Avalonia.Controls Namespace</a>  
