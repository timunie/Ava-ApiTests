---
title:PastingFromClipboard Event
description: Autogenertated API Reference page
hide_table_of_contents: false
---
# PastingFromClipboard Event




## Definition
**Namespace:** <a href="N_Avalonia_Controls">Avalonia.Controls</a>  
**Assembly:** Avalonia.Controls (in Avalonia.Controls.dll) Version: 11.2.4

**C#**
``` C#
public event EventHandler<RoutedEventArgs> PastingFromClipboard
```
**VB**
``` VB
Public Event PastingFromClipboard As EventHandler(Of RoutedEventArgs)
```
**F#**
``` F#
member PastingFromClipboard : IEvent<EventHandler<RoutedEventArgs>,
    RoutedEventArgs>
```



#### Value
<a href="https://learn.microsoft.com/dotnet/api/system.eventhandler-1" target="_blank" rel="noopener noreferrer">EventHandler</a>(<a href="T_Avalonia_Interactivity_RoutedEventArgs">RoutedEventArgs</a>)

## See Also


#### Reference
<a href="T_Avalonia_Controls_TextBox">TextBox Class</a>  
<a href="N_Avalonia_Controls">Avalonia.Controls Namespace</a>  
