---
title:PointerTouchPadGestureMagnifyEvent Field
description: Autogenertated API Reference page
hide_table_of_contents: false
---
# PointerTouchPadGestureMagnifyEvent Field




## Definition
**Namespace:** <a href="N_Avalonia_Input">Avalonia.Input</a>  
**Assembly:** Avalonia.Base (in Avalonia.Base.dll) Version: 11.2.4

**C#**
``` C#
public static readonly RoutedEvent<PointerDeltaEventArgs> PointerTouchPadGestureMagnifyEvent
```
**VB**
``` VB
Public Shared ReadOnly PointerTouchPadGestureMagnifyEvent As RoutedEvent(Of PointerDeltaEventArgs)
```
**F#**
``` F#
static val PointerTouchPadGestureMagnifyEvent: RoutedEvent<PointerDeltaEventArgs>
```



#### Field Value
<a href="T_Avalonia_Interactivity_RoutedEvent_1">RoutedEvent</a>(<a href="T_Avalonia_Input_PointerDeltaEventArgs">PointerDeltaEventArgs</a>)

## See Also


#### Reference
<a href="T_Avalonia_Input_Gestures">Gestures Class</a>  
<a href="N_Avalonia_Input">Avalonia.Input Namespace</a>  
