# UnsetValue Field


A binding notification representing <a href="F_Avalonia_AvaloniaProperty_UnsetValue">UnsetValue</a>.



## Definition
**Namespace:** <a href="N_Avalonia_Data">Avalonia.Data</a>  
**Assembly:** Avalonia.Base (in Avalonia.Base.dll) Version: 11.2.0

<Tabs groupId="api-code-preview">
<TabItem value="csharp" label="C#">
```csharp
public static readonly BindingNotification UnsetValue
```
</TabItem>
<TabItem value="vb" label="VB">
```vb
Public Shared ReadOnly UnsetValue As BindingNotification
```
</TabItem>
<TabItem value="fsharp" label="F#">
```fsharp
static val UnsetValue: BindingNotification
```
</TabItem>
</Tabs>



<a href="https://github.com/AvaloniaUI/Avalonia/tree/master/src/Avalonia.Base/Data/BindingNotification.cs" title="View the source code">View Source</a>



#### Field Value
<a href="T_Avalonia_Data_BindingNotification">BindingNotification</a>

## See Also


#### Reference
<a href="T_Avalonia_Data_BindingNotification">BindingNotification Class</a>  
<a href="N_Avalonia_Data">Avalonia.Data Namespace</a>  

