# BeginningEdit Event


Occurs before a cell or row enters editing mode.



## Definition
**Namespace:** <a href="N_Avalonia_Controls">Avalonia.Controls</a>  
**Assembly:** Avalonia.Controls.DataGrid (in Avalonia.Controls.DataGrid.dll)

<Tabs groupId="api-code-preview">
<TabItem value="csharp" label="C#">
```csharp
public event EventHandler<DataGridBeginningEditEventArgs> BeginningEdit
```
</TabItem>
<TabItem value="vb" label="VB">
```vb
Public Event BeginningEdit As EventHandler(Of DataGridBeginningEditEventArgs)
```
</TabItem>
<TabItem value="fsharp" label="F#">
```fsharp
member BeginningEdit : IEvent<EventHandler<DataGridBeginningEditEventArgs>,
    DataGridBeginningEditEventArgs>
```
</TabItem>
</Tabs>



<a href="https://github.com/AvaloniaUI/Avalonia/tree/master/src/Avalonia.Controls.DataGrid/DataGrid.cs" title="View the source code">View Source</a>



#### Value
<a href="https://learn.microsoft.com/dotnet/api/system.eventhandler-1" target="_blank" rel="noopener noreferrer">EventHandler</a>(<a href="T_Avalonia_Controls_DataGridBeginningEditEventArgs">DataGridBeginningEditEventArgs</a>)

## See Also


#### Reference
<a href="T_Avalonia_Controls_DataGrid">DataGrid Class</a>  
<a href="N_Avalonia_Controls">Avalonia.Controls Namespace</a>  

